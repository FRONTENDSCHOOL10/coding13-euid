...
async function renderChatContents() {
  ...
  // 채팅상대 식별
  const opponent = data.sender_id === currentUserId ? data.receiver_id : data.sender_id;

  // 헤더 정보 렌더링
  opponentName.innerText =
    data.sender_id === currentUserId ? data.expand.receiver_id.username : data.expand.sender_id.username;
  // 판매글 정보 렌더링
...
renderChatContents()
  ...
  .then(({ data, opponent }) => {
    ...
    sendBtn.addEventListener('click', (e) => {
      e.preventDefault();
      sendMessage(chat_id, currentUserId, opponent, messageInput.value);
    });
  });
